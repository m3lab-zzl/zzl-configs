...
if [[ -r "${XDG_CACHE_HOME:-$SCRIPTPATH/.cache}/p10k-instant-prompt-${(%):-%n}.zsh" ]]; then
  source "${XDG_CACHE_HOME:-$SCRIPTPATH/.cache}/p10k-instant-prompt-${(%):-%n}.zsh"
fi

export ZSH="$SCRIPTPATH/.omz"
...
__conda_setup="$("${dire}/bin/conda" 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "${dire}/etc/profile.d/conda.sh" ]; then
    . "${dire}/etc/profile.d/conda.sh"
    else
	export PATH="${dire}/bin:$PATH"
    fi
fi
unset __conda_setup
if [ -f "${dire}/etc/profile.d/mamba.sh" ]; then
    . "${dire}/etc/profile.d/mamba.sh"
fi
...
source $SCRIPTPATH/generalSetting.sh
[[ ! -f $SCRIPTPATH/.p10k.zsh ]] || source $SCRIPTPATH/.p10k.zsh
